...
def xv_t_graph(h, x0, v0, Tmax, approx_type, filename):
...
def err_plot(h, x0, v0, Tmax, approx_type, filename):
    '''Generate a plot of the error on x for the desired method.'''
...
def longper_xv_t_graph(h, x0, v0, Tmax, approx_type, filename):
    '''Plot x and v against t.  Save a plot with the desired filename.'''
    ...
    # Graph the t, x, and v.  Label the plot.
    plt.plot(t_list, x_list, label="Numeric Approximation")
    plt.plot(ta_list, xa_list, label="Analytic Values")
    plt.xlim(990, 1000)
    plt.legend(loc="upper left")
    plt.xlabel("t")
    plt.ylabel("x(t)")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 6:
        plot_type = sys.argv[1][:2]
        approx_type = sys.argv[1][3:6]
        ...
        if plot_type == "xv":
            xv_t_graph(h, x0, v0, Tmax, approx_type, filename)

        if plot_type == "er":
            err_plot(h, x0, v0, Tmax, approx_type, filename)

        if plot_type == "he":
            h_err_plot(h, x0, v0, Tmax, approx_type, filename)

        if plot_type == "en":
            energy_plot(h, x0, v0, Tmax, approx_type, filename)

        if plot_type == "ph":
            phase_plot(h, x0, v0, Tmax, approx_type, filename)
